...
export function ContactSchoolForm() {
  const { register } = useAuth();

  const isMountedRef = useIsMountedRef();

  const RegisterSchema = Yup.object().shape({
    school_name: Yup.string().required("School name required"),
    pincode: Yup.string().required("Pincode required"),
    city: Yup.string().required("City name required"),
    cirruculum: Yup.string().required("Cirruculum name required"),
    no_of_students: Yup.string().required("Number of students required"),
 ...
 const onSubmit = async (data: FormValuesProps) => {
    try {
      ...
      const apibody = {
        ...
        cuNoofStudents: data.no_of_students,
